        ...
        <Flex direction="column" className="h-full">
          ...
          <Box className="flex-grow overflow-y-auto p-4">
            {cartItems.length === 0 ? (
              ...
              cartItems.map((item) => (
                <Flex key={`${item._id}-${item.size}`} className="mb-4 border-b pb-4">
                  <Box className="w-1/4 mr-4">
                    <img src={item.imgUrl} alt={item.name} className="w-full h-auto object-contain" />
                  </Box>
                  <Flex direction="column" className="flex-grow">
                    <Flex justify="between">
                      <Flex direction="column">
                        <Text weight="bold" size="2">{item.name}</Text>
                        <Text size="1" style={{ color: DIM_COLOR }}>{item.colorway}</Text>
                    ...
                    <Flex justify="between" align="center" mt="2">
                      <Flex align="center">
                        ...
                        <Text mx="2" size="2">{item.quantity}</Text>
                        <IconButton 
                          variant="outline" 
                          onClick={() => updateQuantity(item._id, item.size, item.quantity + 1)} 
                          size="1"
                      ...
                      <Text weight="bold" size="2">${(item.price * item.quantity).toFixed(2)}</Text>
                    </Flex>
                  </Flex>
                </Flex>
              ))
          ...
          <Box p="4" className="border-t">
            <Flex justify="between" align="center" mb="2">
              ...
              <Text size="3" weight="bold">${cartTotal.toFixed(2)}</Text>
            </Flex>
            <Button 
