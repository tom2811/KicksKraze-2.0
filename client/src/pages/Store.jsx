...
function Store() {
  ...
  const fetchSneakers = useCallback(async () => {
    ...
    try {
      ...
      if (state.exactId) {
        ...
        data = await getAllSneakers(state.currentPage, ITEMS_PER_PAGE, brandsParam, state.sortOrder, state.searchQuery);
      }
      updateState({
  ...
  }, [state.currentPage, state.selectedBrands, state.sortOrder, state.searchQuery, state.exactId, state.isInitialLoad]);
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const uniqueBrands = [...new Set(sneakers.map(sneaker => sneaker.brand))];
        updateState({ brands: uniqueBrands });
      } catch (error) {
  ...
  }, [state.searchQuery, state.exactId, state.currentPage, state.selectedBrands, state.sortOrder, navigate, state.isInitialLoad]);

  // Handler for brand selection
  const handleBrandChange = (brand) => {
    updateState({
      selectedBrands: brand === 'All Items' 
        ? [] 
  ...
  // Handler for adding item to cart (to be implemented)
  const handleAddToCart = (sneaker) => {
    console.log('Added to cart:', sneaker);
  };

  // Toggle filter menu visibility (for mobile)
...
function FilterSection({ brands, selectedBrands, isFilterOpen, handleBrandChange, toggleFilter }) {
  return (
    ...
    <Box className="w-full md:w-3/4 lg:w-10/12">
      ...
      <Flex justify="center" mt="5">
        <Flex align="center" gap="2">
          ...
          <Text size="1" className="text-gray-600 md:text-xs lg:text-sm">Page {state.currentPage} of {state.totalPages}</Text>
          <Button 
